...
#### BBraun 최소 표시 모드 (1.18.29+)

여러 펌프가 동시에 동작하는 실험에서 화면 어수선함을 줄이려면, `vr.conf` 의 BBraun 장비 섹션에 `minimal=1` 을 추가합니다. 펌프별로 세 개의 핵심 트랙 — `PUMP{N}_DRUG` (약물명), `PUMP{N}_RATE` (주입 속도, mL/h), `PUMP{N}_VOL` (누적 주입량, mL) — 만 등록되며 그 외 필드(압력, 농도, 용량 속도, 주사기, bolus, 주입 시간, 환자 체중, drug library, care area 등) 는 표시·기록에서 제외됩니다. 자동 생성된 침상 탭도 primary 탭의 설정을 상속받습니다.
...
BBraun HL7 (DoseLink) 및 BBraun SpaceCom (RS-232) 양쪽 모두 지원됩니다.
