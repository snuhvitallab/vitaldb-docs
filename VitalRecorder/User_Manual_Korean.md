...
2. **침상명 기반 라우팅 (권장, 키워드 필터보다 우선)** — 각 탭의 Bed Name을 게이트웨이가 보내는 식별자 중 **아무거나 하나**와 정확히 일치시키면 필터 설정 없이 자동 매칭됩니다:
   - **Mindray**: `PV1-3` 침상 ID (예: `SU-1`, `BED-001`)
   - **Nihon Kohden**: `deviceId` JSON 필드 또는 12바이트 MFER prefix
   - **BBraun**: `MDC_ATTR_LOCATION` OBX 값의 `~` 구분자로 분리된 non-empty 토큰 중 **아무거나** (예: `Forskning`, `Bord4`, `Anilab`, `Operasjon`)

   같은 프레임에 여러 탭이 매칭 가능한 경우(예: `Forskning` 탭과 `Bord4` 탭이 모두 있고 프레임의 LOCATION이 `Forskning~Operasjon~Bord4~Anilab~~~~~Bord4`이면) **더 구체적인 토큰이 우선**합니다. 토큰은 역순(마지막부터)으로 검사되므로 `Bord4`(LOCATION 끝의 display short-name)가 `Forskning`(앞쪽의 부서명)보다 우선합니다. 하나의 프레임은 오직 하나의 탭에만 전달됩니다.

3. **키워드 필터 라우팅 (대체)** — 탭 이름을 게이트웨이 식별자와 다르게 짓고 싶을 때는 위의 `포트#키워드` 문법을 사용합니다. 침상명 기반 라우팅이 실패한 경우에만 적용됩니다.

4. **자동 탭 생성** — 기존 어떤 탭과도 매칭되지 않고 프레임에 침상 식별자가 있으면 **가장 구체적인 토큰**(마지막 non-empty)으로 새 탭이 자동 생성됩니다. 초기 설정 누락 시 패킷 손실을 막는 용도입니다.
...
BBraun DoseLink의 경우 하나의 HL7 프레임이 하나의 rack(=한 침상)을 나타내며, rack 안의 여러 펌프는 VMD 블록으로 프레임 안에 포함되어 같은 탭의 별도 트랙(PUMP1 … PUMP16)으로 기록됩니다.

#### Monitor View 의 펌프 표시 (1.18.30+)

Monitor View 는 이제 **8개 펌프까지 동시 표시**합니다 (이전 4개). 각 슬롯에는 약물명과 큰 값이 표시됩니다:

- **TCI 펌프 (effect-site concentration, CE 제공)** — 슬롯에 `CE` + 약물명 (기존 동작 유지).
- **비 TCI 펌프 (대부분의 BBraun / Fresenius 설정)** — 슬롯에 `RATE` (mL/h) + 약물명이 표시되고, 누적 주입량 (`VOL`, mL) 은 슬롯 우상단에 작은 회색 보조 텍스트로 표시됩니다.

그 외 모든 펌프 필드 (압력, 농도, 용량 속도, 주사기, bolus, 주입 시간, 환자 체중, drug library, care area 등) 는 여전히 **`.vital` 파일에 기록**되며 기본 트랙 뷰에서 확인할 수 있습니다. Monitor View 만 실험 중 화면 어수선함을 줄이도록 curation 합니다.

지원 장비: BBraun SpaceCom/HL7, Fresenius Agilia/Primea/PCBM, Daiwha, Pion. 별도 설정 불필요 — 이 레이아웃이 기본값입니다.

> 1.18.29 에 도입된 `minimal=1` vr.conf 옵션은 1.18.30 에서 제거되었습니다. 해당 옵션은 `.vital` 파일에서도 해당 필드를 누락시켜 사후 복구가 불가능했습니다. 1.18.30 의 Monitor View 재설계로 모든 기록 데이터가 보존됩니다.

> **1.18.23 영문이 아닌 Windows 사용자 안내** — 이전 버전에서는 `,`를 소수점 구분자로 쓰는 locale(노르웨이어, 독일어, 프랑스어 등) Windows에서 1.0 mL/h 미만의 주입 속도가 0으로 기록되는 C 런타임 이슈가 있었습니다. 1.18.23부터 Windows 지역 설정과 무관하게 항상 `.`을 소수점 구분자로 해석합니다. BBraun 펌프 한계도 장비당 8 → 16개로 확장되었습니다.
